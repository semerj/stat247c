...
> The coefficient estimates between naive and robust regressions are the same, while the standard errors are slightly different, but not by much. When adding a longitudinal term (i.e. `time`), the estimates for each treatment don't seem to change much in comparison to the simple cross-sectional model. The `time` estimate itself is small and not significant.

```{r, message=FALSE}
library(sandwich)
library(lmtest)

naive_model = lm(y ~ tx, data = df)
# naive SE
...
longitudinal_model = lm(y ~ tx + time, data = df)
# naive SE
